...
class DataExtraction:
    ...
    writes the data in to Datframes, excel and csv files.
    """
    ...
    def get_connection_str(_self,path,file_name):
        """In this method I used modules to read
            the pdf's that are mentioned in the task. As there is some Data loss in case of tabulate for 2nd pdf. 
    ...
    def get_filename(_self,invoice_file_name):
        "Useful to extract the file name with out extension"
    ...
    def convert_date(format_type,date_time):
        ...
        pdf's in to common format which is 'Date-Month-YYYY' format
        """
    ...
    def format_invoice2(_self,abs_filePath,abs_fileName):
        """Interated line by Line and checked whether the Total USD and 
    ...
    def get_data(_self):
        ...
        pdf1_invoicedf=pd.DataFrame({
                        "File Name":[file_name],
                        "Date":[format_date],
                        "Value":[gross_amount]
                     })
        print("------------------Printing Dataframe of sample invoice pdf1--------")
        print(pdf1_invoicedf)
        ...
        pdf2_invoicedf=_self.format_invoice2(abs_filePath,abs_fileName)
        print("------------------Printing Dataframe of sample invoice pdf2--------")
        print(pdf2_invoicedf)
        merged_df=pd.concat([pdf1_invoicedf,pdf2_invoicedf],axis=0,ignore_index=True)
        ...
        """
        with pd.ExcelWriter('Output\invoice_excel.xlsx') as writer:
        ...
        merged_df.to_csv("Output\Total.csv",sep=';')
